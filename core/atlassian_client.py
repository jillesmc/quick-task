"""
Cliente Atlassian - Wrapper para REST API v3 do Jira/Atlassian.
"""
...
class AtlassianClient:
    """Cliente para interagir com Atlassian (Jira) via REST API v3."""
    ...
    def _text_to_adf(text: str) -> Dict[str, Any]:
        ...
        lines = text.split("\n")
        debug_log(
            "AtlassianClient", "_text_to_adf", "Texto dividido em %d linhas", len(lines)
        )
        content = []
        i = 0
        in_code_block = False
        ...
        while i < len(lines):
            ...
            if stripped:
                ...
                content.append(
                    {
                        "type": "paragraph",
                        "content": AtlassianClient._parse_inline_formatting(stripped),
                    }
                )
                i += 1
            else:
    ...
    def _adf_to_markdown(adf_node: Any) -> str:
        ...
        if isinstance(adf_node, list):
            return "\n\n".join(
                AtlassianClient._adf_to_markdown(item) for item in adf_node
            ).strip()
        ...
        if node_type == "doc":
            ...
            parts = [AtlassianClient._adf_to_markdown(c).strip() for c in content]
        ...
        if node_type == "heading":
            ...
            prefix = "#" * level + " "
            inline = "".join(
                AtlassianClient._adf_inline_to_markdown(c) for c in content
            ).strip()
        ...
        if node_type == "codeBlock":
            ...
            for c in content:
                if isinstance(c, dict) and c.get("type") == "text":
                    ...
                    code_parts.append(AtlassianClient._adf_to_markdown(c))
        ...
        if node_type == "text":
            return AtlassianClient._adf_text_node_to_markdown(adf_node)
        ...
        if content:
            return (
                "\n\n".join(
                    AtlassianClient._adf_to_markdown(c).strip() for c in content
                ).strip()
                + "\n"
            )
        return ""
    ...
    def _adf_inline_to_markdown(inline_node: Any) -> str:
        ...
        if isinstance(inline_node, dict):
            ...
            if inline_node.get("type") == "text":
                return AtlassianClient._adf_text_node_to_markdown(inline_node)
        return ""
    ...
    def create_issue(
        ...
        if description:
            ...
            debug_log("AtlassianClient", "create_issue", "Conversão ADF concluída")
        ...
        try:
            ...
            debug_log("AtlassianClient", "create_issue", "ERRO - %s", e)
            raise
        except Exception as e:
            debug_log("AtlassianClient", "create_issue", "ERRO inesperado - %s", e)
        ...
        while True:
            ...
            for w in worklogs:
                ...
                if isinstance(body_raw, dict):
                    body_md = AtlassianClient._adf_to_markdown(body_raw)
                else:
