...
class TimerService(QObject):
    ...
    def __init__(
        ...
        self._break_decision_timeout_timer.timeout.connect(
            self._on_break_decision_timeout
        )
    ...
    def _load_pomodoro_config(self) -> None:
        ...
        self._pomodoro_duration_seconds = (
            pomodoro_config.get("pomodoro_duration_minutes", 25) * 60
        )
        ...
        debug_log(
            ...
            "Configurações carregadas: enabled=%s, duration=%ds",
            self._pomodoro_enabled,
            self._pomodoro_duration_seconds,
        )
    ...
    def start(self, issue_key: str) -> None:
        ...
        if self._is_running or self._timer_model.state == TimerState.RUNNING.value:
            debug_log(
                "TimerService",
                "start",
                "Timer já está rodando - estado: %s, is_running: %s",
        ...
        _write_debug_log(
            {
                ...
                "runId": "run1",
                "hypothesisId": "E",
                "location": "timer_service.py:102",
                "message": "start() - antes de inicializar currentPomodoro",
                "data": {
                    "accumulated_pomodoros": accumulated_pomodoros,
                    "current_before": self._timer_model._current_pomodoro,
                },
            ...
            _write_debug_log(
                {
                    ...
                    "runId": "run1",
                    "hypothesisId": "E",
                    "location": "timer_service.py:117",
                    "message": "start() - atribuindo currentPomodoro (accumulated > 0)",
                    "data": {
                        "old": self._timer_model._current_pomodoro,
                        "new": accumulated_pomodoros,
                    },
                    "timestamp": int(time.time() * 1000),
                }
            )
            ...
            _write_debug_log(
                {
                    ...
                    "runId": "run1",
                    "hypothesisId": "E",
                    "location": "timer_service.py:124",
                    "message": "start() - atribuindo currentPomodoro (primeira vez)",
                    "data": {"old": self._timer_model._current_pomodoro, "new": 0},
                    "timestamp": int(time.time() * 1000),
                }
            )
            # #endregion
        ...
        _write_debug_log(
            {
                ...
                "runId": "run1",
                "hypothesisId": "E",
                "location": "timer_service.py:116",
                "message": "start() - depois de inicializar currentPomodoro",
                "data": {
                    "accumulated_pomodoros": accumulated_pomodoros,
                    "current_after": self._timer_model._current_pomodoro,
                },
                "timestamp": int(time.time() * 1000),
            }
        )
        ...
        if self._timer_model.state != TimerState.RUNNING.value:
            debug_log(
                "TimerService",
                "start",
                "Erro: estado não mudou para RUNNING após start_session. Estado atual: %s",
                self._timer_model.state,
            )
            return
        ...
        debug_log(
            "TimerService",
            "start",
            "Timer iniciado com sucesso - estado: %s, is_running: %s, timer ativo: %s",
            self._timer_model.state,
            self._is_running,
    ...
    def pause(self) -> None:
            ...
            if session and self._worklog_db:
                try:
                    ...
                    debug_log(
                        "TimerService",
                        "pause",
                        "Worklog salvo antes de pausar: %s",
                        session.id,
                    )
        ...
        debug_log(
            "TimerService",
            "pause",
            "Pausa iniciada: tipo=%s, duração=%ds",
            break_type,
    ...
    def resume(self) -> None:
        ...
        if self._timer_model.isWaitingBreakEndDecision:
            debug_log(
                "TimerService",
                "resume",
                "Pausa terminou, continuando via continueAfterBreak",
            )
        ...
        debug_log(
            "TimerService",
            "resume",
            "Resume não é mais suportado - pausas usam cronômetro",
        )
    ...
    def stop(self) -> None:
        ...
        if session and self._worklog_db:
            try:
                ...
                debug_log(
                    "TimerService", "stop", "Sessão salva no banco: %s", session.id
                )
                self._timer_model.sessionSaved.emit(session.id)
            except Exception as e:
        ...
        if issue_key and issue_key in self._pomodoro_counters:
            debug_log(
                ...
                issue_key,
                self._pomodoro_counters[issue_key],
            )
    ...
    def _on_tick(self) -> None:
        ...
        _write_debug_log(
            {
                ...
                "runId": "run1",
                "hypothesisId": "A",
                "location": "timer_service.py:321",
                "message": "_on_tick() - entrada do bloco pomodoro",
                "data": {
                    "pomodoro_enabled": self._pomodoro_enabled,
                    "total_elapsed": total_elapsed,
                    "current_pomodoro_before": self._timer_model._current_pomodoro,
                },
                "timestamp": int(time.time() * 1000),
            }
        )
        ...
        if self._pomodoro_enabled and total_elapsed > 0:
            ...
            pomodoros_in_current_session = (
                total_elapsed // self._pomodoro_duration_seconds
            )
            ...
            _write_debug_log(
                {
                    ...
                    "runId": "run1",
                    "hypothesisId": "D",
                    "location": "timer_service.py:332",
                    "message": "_on_tick() - calculado current_pomodoro_number",
                    "data": {
                        "accumulated_pomodoros": accumulated_pomodoros,
                        "pomodoros_in_current_session": pomodoros_in_current_session,
                        "current_pomodoro_number": current_pomodoro_number,
                        "current_pomodoro_model": self._timer_model._current_pomodoro,
                    },
                    "timestamp": int(time.time() * 1000),
                }
            )
            ...
            if pomodoros_in_current_session > self._last_pomodoro_detected_in_session:
                ...
                pomodoro_start = pomodoro_start.replace(second=0, microsecond=0)
                ...
                pomodoro_start_offset = (
                    pomodoros_in_current_session - 1
                ) * self._pomodoro_duration_seconds
                pomodoro_start = pomodoro_start.replace(
                    second=(total_elapsed - pomodoro_start_offset)
                    % self._pomodoro_duration_seconds
                )
                ...
                _write_debug_log(
                    {
                        ...
                        "runId": "run1",
                        "hypothesisId": "A",
                        "location": "timer_service.py:393",
                        "message": "_on_tick() - atribuindo currentPomodoro após completar",
                        "data": {
                            "old": self._timer_model._current_pomodoro,
                            "new": new_pomodoro,
                        },
                ...
                debug_log(
                    "TimerService",
                    "_on_tick",
                    "Pomodoro %d completado, break_type=%s",
                    new_pomodoro,
                    break_type,
                )
            else:
                ...
                _write_debug_log(
                    {
                        ...
                        "runId": "run1",
                        "hypothesisId": "A",
                        "location": "timer_service.py:396",
                        "message": "_on_tick() - antes de atualizar currentPomodoro (não completou novo)",
                        "data": {
                            "current_pomodoro_number": current_pomodoro_number,
                            "current_pomodoro_model": self._timer_model._current_pomodoro,
                            "will_update": current_pomodoro_number
                            != self._timer_model.currentPomodoro,
                        },
                        "timestamp": int(time.time() * 1000),
                    }
                )
                    ...
                    _write_debug_log(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "A",
                            "location": "timer_service.py:419",
                            "message": "_on_tick() - atribuindo currentPomodoro (não completou novo)",
                            "data": {
                                "old": self._timer_model._current_pomodoro,
                                "new": current_pomodoro_number,
                            },
                            "timestamp": int(time.time() * 1000),
                        }
                    )
                    ...
                    _write_debug_log(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "A",
                            "location": "timer_service.py:422",
                            "message": "_on_tick() - depois de atualizar currentPomodoro",
                            "data": {
                                "current_pomodoro_after": self._timer_model._current_pomodoro
                            },
                            "timestamp": int(time.time() * 1000),
                        }
                    )
    ...
    def reload_config(self) -> None:
        ...
        if old_pomodoro_duration != self._pomodoro_duration_seconds:
            changed.append(
                f"pomodoro_duration: {old_pomodoro_duration}s -> {self._pomodoro_duration_seconds}s"
            )
        if old_auto_continue_timeout != self._auto_continue_timeout_seconds:
            changed.append(
                f"auto_continue_timeout: {old_auto_continue_timeout}s -> {self._auto_continue_timeout_seconds}s"
            )
        if old_short_break != self._short_break_seconds:
            changed.append(
                f"short_break: {old_short_break}s -> {self._short_break_seconds}s"
            )
        if old_long_break != self._long_break_seconds:
            changed.append(
                f"long_break: {old_long_break}s -> {self._long_break_seconds}s"
            )
        if old_pomodoros_before_long != self._pomodoros_before_long_break:
            changed.append(
                f"pomodoros_before_long: {old_pomodoros_before_long} -> {self._pomodoros_before_long_break}"
            )
    ...
    def _count_short_breaks_in_session(self) -> int:
        ...
        debug_log(
            "TimerService",
            "_count_short_breaks_in_session",
            "Pausas curtas na sessão: %d",
            count,
        )
        return count
    ...
    def _determine_break_type(self) -> str:
        ...
        if (
            ...
            and pomodoros_completed % self._pomodoros_before_long_break == 0
        ):
            break_type = "long"
            debug_log(
                "TimerService",
                ...
                pomodoros_completed,
                self._pomodoros_before_long_break,
            )
        else:
            break_type = "short"
            debug_log(
    ...
    def acceptBreak(self, break_type: str) -> None:
        ...
        debug_log(
            "TimerService",
            "acceptBreak",
            "Aceitando pausa após Pomodoro, tipo=%s",
            break_type,
        )
        ...
        if self._timer_model._current_session:
            ...
            if session and self._worklog_db:
                try:
                    ...
                    debug_log(
                        "TimerService", "acceptBreak", "Erro ao salvar sessão: %s", e
                    )
            ...
            if self._current_break_session:
                ...
                if self._current_break_session.break_type == "long":
                    self._current_break_session.duration_seconds = (
                        self._long_break_seconds
                    )
                else:
                    self._current_break_session.duration_seconds = (
                        self._short_break_seconds
                    )
    ...
    def _on_break_decision_timeout(self) -> None:
        ...
        debug_log(
            "TimerService",
            "_on_break_decision_timeout",
            "Timeout atingido, continuando automaticamente",
        )
    ...
    def continueAfterBreak(self) -> None:
        ...
        if issue_key:
            ...
            debug_log(
                "TimerService", "continueAfterBreak", "Sem issue key para continuar"
            )
    ...
    def cancelBreak(self) -> None:
        ...
        if self._break_countdown_timer.isActive():
            self._break_countdown_timer.stop()
            debug_log(
                "TimerService", "cancelBreak", "Timer de contagem regressiva parado"
            )
        ...
        if self._current_break_session and self._timer_model._current_session:
            try:
                self._timer_model._current_session.pomodoros.remove(
                    self._current_break_session
                )
                debug_log(
                    "TimerService",
                    "cancelBreak",
                    "PomodoroSession da pausa removida da sessão",
                )
            except ValueError:
                debug_log(
                    "TimerService",
                    "cancelBreak",
                    "PomodoroSession da pausa não encontrada na lista",
                )
            self._current_break_session = None
