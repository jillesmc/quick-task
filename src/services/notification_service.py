...
class NotificationService(QObject):
        ...
        if MEDIA_PLAYER_AVAILABLE:
            try:
                ...
                debug_log(
                    "NotificationService",
                    "__init__",
                    "Erro ao inicializar player de áudio: %s",
                    e,
                )
                self._sound_player = None
                self._audio_output = None
    ...
    def show_pomodoro_notification(
        ...
        debug_log(
            "NotificationService",
            "show_pomodoro_notification",
            "Timer de auto-continuação iniciado: %ds",
    ...
    def _on_auto_continue(self) -> None:
        ...
        debug_log(
            "NotificationService", "_on_auto_continue", "Timeout atingido, continuando"
        )
    ...
    def cancel_auto_continue(self) -> None:
        ...
        self._auto_continue_timer.stop()
        debug_log(
            "NotificationService", "cancel_auto_continue", "Auto-continuação cancelada"
        )
    ...
    def _find_sound_file(self, break_type: Optional[str] = None) -> Optional[Path]:
        ...
        if break_type and self._settings_model:
            if break_type == "short":
                ...
                debug_log(
                    "NotificationService",
                    "_find_sound_file",
                    "Pausa curta: usando arquivo/caminho: %s",
                    filename_or_path,
                )
                ...
                debug_log(
                    "NotificationService",
                    "_find_sound_file",
                    "Pausa longa: usando arquivo/caminho: %s",
                    filename_or_path,
                )
            else:
        ...
        if sound_path.is_absolute() and sound_path.exists():
            debug_log(
                "NotificationService",
                "_find_sound_file",
                "Usando caminho completo fornecido: %s (break_type=%s)",
                sound_path,
                break_type,
            )
            return sound_path
        ...
        if sound_path.suffix:
            ...
            filename = (
                sound_path.name
                if sound_path.name
                else sound_path.stem if sound_path.stem else str(sound_path)
            )

        # Possíveis locais para assets
        possible_paths = [
            Path(__file__).parent.parent.parent.parent
            / "assets",  # src/services/../../assets (desenvolvimento)
            ...
            for fmt in formats:
                ...
                if sound_file.exists():
                    debug_log(
                        "NotificationService",
                        "_find_sound_file",
                        "Arquivo de som encontrado: %s (break_type=%s)",
                        sound_file,
                        break_type,
                    )
                    return sound_file
        ...
        debug_log(
            "NotificationService",
            "_find_sound_file",
            "Nenhum arquivo de som encontrado para '%s' nos assets",
            filename,
        )
        return None
    ...
    def play_pomodoro_sound(self, break_type: Optional[str] = None) -> None:
        ...
        if sound_file and MEDIA_PLAYER_AVAILABLE and self._sound_player:
            try:
                ...
                debug_log(
                    "NotificationService",
                    "play_pomodoro_sound",
                    "Tocando arquivo de som: %s (break_type=%s)",
                    sound_file,
                    break_type,
                )
                return
                ...
                debug_log(
                    "NotificationService",
                    "play_pomodoro_sound",
                    "Erro ao tocar arquivo de som: %s",
                    e,
                )
        ...
        try:
            ...
            debug_log(
                "NotificationService",
                "play_pomodoro_sound",
                "Som tocado (beep do sistema)",
            )
        except Exception as e:
            debug_log(
                "NotificationService", "play_pomodoro_sound", "Erro ao tocar som: %s", e
            )
