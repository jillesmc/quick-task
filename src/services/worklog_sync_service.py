    ...
    def run(self):
        ...
        for i, session_id in enumerate(self._session_ids):
            try:
                ...
                if not session:
                    debug_log(
                        "SyncWorker", "run", "Sessão %s não encontrada", session_id
                    )
                ...
                debug_log(
                    "SyncWorker",
                    "run",
                    "Enviando worklog para Jira: issue=%s, duration=%s",
                    session["issue_key"],
                    time_spent,
                )
                ...
                if success:
                    ...
                    debug_log(
                        "SyncWorker",
                        "run",
                        "Worklog sincronizado com sucesso: %s",
                        session_id,
                    )
                else:
                    ...
                    debug_log(
                        "SyncWorker", "run", "Erro ao sincronizar: %s", session_id
                    )
                ...
                self.sessionError.emit(session_id, error_msg)
                debug_log(
                    "SyncWorker", "run", "Exceção ao sincronizar %s: %s", session_id, e
                )
...
class WorklogSyncService(QObject):
    ...
    def get_pending_worklogs(self):
        ...
        debug_log(
            ...
            "Retornando %d worklogs pendentes",
            len(pending),
        )
        return pending
    ...
    def sync_pending_worklogs(self, session_ids=None) -> bool:
        ...
        if not jira_client:
            debug_log(
                "WorklogSyncService",
                "sync_pending_worklogs",
                "JiraClient não disponível",
            )
            return False
        ...
        if session_ids is None:
            ...
            try:
                session_ids = (
                    list(session_ids)
                    if hasattr(session_ids, "__iter__")
                    and not isinstance(session_ids, str)
                    else [session_ids]
                )
            except:
        ...
        if not session_ids:
            debug_log(
                "WorklogSyncService", "sync_pending_worklogs", "Nenhum worklog pendente"
            )
            return False
        ...
        debug_log(
            "WorklogSyncService",
            "sync_pending_worklogs",
            "Iniciando sincronização de %d worklogs",
            len(session_ids),
        )
    ...
    def _on_sync_finished(self) -> None:
        ...
        debug_log(
            "WorklogSyncService",
            "_on_sync_finished",
            "Sincronização concluída: %d worklogs",
            synced_count,
        )
