...
Controls.Dialog {
    ...
    function _filteredProjects() {
        ...
        for (var i = 0; i < _projects.length; i++) {
            ...
            if (key.indexOf(q) >= 0 || name.indexOf(q) >= 0)
                out.push(p);
        }
        return out;
    }
    ...
    function _stepTitle() {
        ...
        if (currentStep === 3)
            return qsTr("Etapa 4: Configurar campos Assets");
    ...
    function _objectTypesForRow(configKey, fieldId) {
        ...
        for (var i = 0; i < _assetsFieldsList.length; i++) {
            if (_assetsFieldsList[i].configKey === configKey && _assetsFieldsList[i].fieldId === fieldId) {
                ...
                if (!cfg || !cfg.object_schema_id)
                    return [];
                var arr = _objectTypesBySchemaId[cfg.object_schema_id] || [];
                if (typeof console !== "undefined" && console.log) {
                    var keys = [];
                    for (var k in _objectTypesBySchemaId)
                        keys.push(k);
        ...
        for (var pk in sf) {
            ...
            for (var it in byType) {
                ...
                for (var k = 0; k < list.length; k++) {
                    if (list[k].real_type === "Assets objects")
                        return true;
                }
            }
        }
    ...
    function _buildSavePayloadWithAssetsConfig() {
        ...
        for (var i = 0; i < _assetsFieldsList.length; i++) {
            ...
            var cfg = _assetsConfigList[i];
            if (!item || !cfg || !payload.selected_fields[item.projectKey] || !payload.selected_fields[item.projectKey][item.issuetypeId])
                continue;
    ...
    function _discoverIssueTypes() {
        if (!jiraMetadataConfigModel || !_selectedProjectId)
            return;
        _loading = true;
        _errorMessage = "";
    ...
    function _discoverFields() {
        if (!jiraMetadataConfigModel || !_selectedProjectKey || !_selectedIssuetypeId)
            return;
        _loading = true;
        _errorMessage = "";
    ...
    function _buildSavePayload() {
        ...
        for (var i = 0; i < _selectedProjects.length; i++) {
            ...
            payload.selected_projects.push({
                "id": p.id,
                "key": p.key,
                "name": p.name,
                "enabled": true
        ...
        for (var i = 0; i < _issueTypesByProject.length; i++) {
            ...
            for (var j = 0; j < types.length; j++) {
                var t = types[j];
                payload.selected_issue_types[pk].push({
                    "id": t.id,
                    "name": t.name,
                    "enabled": true
                });
        ...
        for (var idx = 0; idx < _fieldConfigList.length; idx++) {
            ...
            if (!c || !c.projectKey || !c.issuetypeId)
                continue;
            var key = _configKey(c.projectKey, c.issuetypeId);
            var fields = _fieldsByConfig[key];
            if (!fields)
            ...
            for (var k = 0; k < fields.length; k++) {
                ...
                if (f.real_type !== undefined)
                    o.real_type = f.real_type;
        ...
        ColumnLayout {
                    ...
                    Controls.ScrollView {
                        ...
                        ListView {
                            ...
                            delegate: Controls.CheckDelegate {
                                ...
                                text: (modelData && (modelData.key || modelData.name)) ? ((modelData.key || "") + (modelData.name ? " – " + modelData.name : "") || modelData.key || modelData.name || qsTr("(sem nome)")) : qsTr("(sem nome)")
                                checked: root._selectedProjects.some(function (p) {
                                    return p.id === modelData.id;
                                })
                                onToggled: {
                                    if (checked) {
                                        if (!root._selectedProjects.some(function (p) {
                                            return p.id === modelData.id;
                                        }))
                                            root._selectedProjects.push(modelData);
                                    } else {
                                        root._selectedProjects = root._selectedProjects.filter(function (p) {
                                            return p.id !== modelData.id;
                                        });
                                    }
                    ...
                    Controls.ScrollView {
                        ...
                        ListView {
                            ...
                            delegate: Controls.CheckDelegate {
                                ...
                                checked: root._selectedIssueTypes.some(function (t) {
                                    return t.id === modelData.id;
                                })
                                onToggled: {
                                    if (checked) {
                                        if (!root._selectedIssueTypes.some(function (t) {
                                            return t.id === modelData.id;
                                        }))
                                            root._selectedIssueTypes.push(modelData);
                                    } else {
                                        root._selectedIssueTypes = root._selectedIssueTypes.filter(function (t) {
                                            return t.id !== modelData.id;
                                        });
                                    }
                    ...
                    Controls.ScrollView {
                        ...
                        ListView {
                            ...
                            delegate: Controls.CheckDelegate {
                                ...
                                checked: root._selectedFields.some(function (f) {
                                    return f.id === modelData.id;
                                })
                                onToggled: {
                                    if (checked) {
                                        if (!root._selectedFields.some(function (f) {
                                            return f.id === modelData.id;
                                        }))
                                            root._selectedFields.push(modelData);
                                    } else {
                                        root._selectedFields = root._selectedFields.filter(function (f) {
                                            return f.id !== modelData.id;
                                        });
                                    }
            ...
            RowLayout {
                ...
                Controls.Button {
                    ...
                    enabled: (root.currentStep === 0 && root._selectedProjects.length > 0) || (root.currentStep === 1 && root._selectedIssueTypes.length > 0) || (root.currentStep === 2 && root._selectedFields.length > 0) || (root.currentStep === 3 && root._assetsFieldsList.length > 0)
                    onClicked: {
                        if (root.currentStep === 0) {
                            ...
                            if (root._fieldConfigList.length > 0) {
                                ...
                                var nextConfig = {};
                                for (var k in byConfig)
                                    nextConfig[k] = byConfig[k];
