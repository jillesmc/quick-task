...
ColumnLayout {
    ...
    RowLayout {
        ...
        Controls.Label {
            ...
            text: selectedEpicKey
                  ? qsTr("Epic selecionada: %1 - %2").arg(selectedEpicKey).arg(selectedEpicSummary)
                  : ""
    ...
    function performSearch(query, isPagination) {
        ...
        jiraService.searchEpicsAsync(
            queryText,
            root.filterCreatedByMe,
            root.filterAssignedToMe,
            root.filterProjectPlatform,
            root.filterExcludeDone,
            nextToken
        )
    }
    ...
    function selectEpic(key, summary) {
        ...
        if (!found) {
            var epic = { key: key, summary: summary || "" }
