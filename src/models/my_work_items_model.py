"""
Modelo para lista de work items do usuário logado.

Expõe uma lista simples de work items para uso em QML, obtida via REST API v3.
"""
...
class MyWorkItemsModel(QObject):
    """Modelo para lista de work items do usuário."""
    ...
    def __init__(self, parent=None) -> None:
        ...
        # Inicializar cliente com o mesmo caminho de config
        try:
            ...
            try:
                ...
                debug_log(
                    "MyWorkItemsModel",
                    "__init__",
                    "Cliente não inicializado: %s (normal se configuração ainda não foi feita)",
                    e,
                )
            except ImportError:
    ...
    def reloadConfiguration(self) -> None:
        """
        Recarrega configuração e recria o cliente.
        ...
        try:
            ...
            # Recriar cliente com nova config
            ...
            try:
                ...
                debug_log(
                    "MyWorkItemsModel",
                    "reloadConfiguration",
                    "Cliente recriado com sucesso",
                )
            except RuntimeError as e:
                debug_log(
                    "MyWorkItemsModel",
                    "reloadConfiguration",
                    "Cliente não pôde ser recriado: %s",
                    e,
                )
                self._jira_client = None
        except Exception as e:
            ...
            debug_log(
                "MyWorkItemsModel", "reloadConfiguration", "Erro ao recarregar: %s", e
            )
    ...
    def refreshIssues(self, query: str = "") -> None:
        ...
        if not self._jira_client:
            self.errorOccurred.emit(
                "Cliente não inicializado. Configure a conexão na aba de Configurações."
            )
            return
    ...
    def _on_issues_found(self, issues: List[Dict[str, Any]]) -> None:
        ...
        debug_log("MyWorkItemsModel", "_on_issues_found", "issuesChanged emitido")
