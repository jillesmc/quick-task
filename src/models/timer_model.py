...
class TimerModel(QObject):
    ...
    isWaitingBreakDecisionChanged = Signal(
        bool
    )  # emitido quando isWaitingBreakDecision muda
    isWaitingBreakEndDecisionChanged = Signal(
        bool
    )  # emitido quando isWaitingBreakEndDecision muda
    ...
    def state(self, value: str):
        try:
            ...
            if self._state != new_state:
                ...
                debug_log(
                    "TimerModel", "state.setter", "Estado alterado para: %s", value
                )
        except ValueError:
            ...
            _write_debug_log(
                {
                    ...
                    "runId": "run1",
                    "hypothesisId": "B",
                    "location": "timer_model.py:175",
                    "message": "currentPomodoro setter - antes de atualizar",
                    "data": {"old_value": self._current_pomodoro, "new_value": value},
            ...
            _write_debug_log(
                {
                    ...
                    "runId": "run1",
                    "hypothesisId": "B",
                    "location": "timer_model.py:178",
                    "message": "currentPomodoro setter - depois de atualizar e emitir signal",
                    "data": {"current_value": self._current_pomodoro},
                    "timestamp": int(time.time() * 1000),
                }
            )
            # #endregion
    ...
    def start_session(self, issue_key: str) -> None:
        ...
        _write_debug_log(
            {
                ...
                "runId": "run1",
                "hypothesisId": "C",
                "location": "timer_model.py:210",
                "message": "start_session() - resetando currentPomodoro para 0",
                "data": {
                    "issue_key": issue_key,
                    "current_before_reset": self._current_pomodoro,
                },
                "timestamp": int(time.time() * 1000),
            }
        )
        ...
        _write_debug_log(
            {
                ...
                "hypothesisId": "C",
                "location": "timer_model.py:210",
                "message": "start_session() - depois de resetar currentPomodoro",
                "data": {"current_after_reset": self._current_pomodoro},
                "timestamp": int(time.time() * 1000),
            }
        )
        # #endregion
        ...
        debug_log(
            "TimerModel", "start_session", "Sessão iniciada para issue: %s", issue_key
        )
        ...
        _write_debug_log(
            {
                ...
                "runId": "run1",
                "hypothesisId": "C",
                "location": "timer_model.py:271",
                "message": "stop_session() - resetando currentPomodoro para 0",
                "data": {"current_before": self._current_pomodoro},
                "timestamp": int(time.time() * 1000),
            }
        )
        # #endregion
    ...
    def add_pomodoro(self, pomodoro: PomodoroSession) -> None:
        ...
        if self._current_session:
            ...
            debug_log(
                "TimerModel",
                "add_pomodoro",
                "Pomodoro adicionado à sessão (total na sessão: %d)",
                len(self._current_session.pomodoros),
            )
