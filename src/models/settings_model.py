...
class SettingsModel(QObject):
    ...
    def _load_current_values(self):
        ...
        debug_log("SettingsModel", "_load_current_values", "Carregando valores de .jira-config.yml")
        if jira_config_path and jira_config_path.exists():
            debug_log("SettingsModel", "_load_current_values", "Arquivo encontrado: %s", jira_config_path)
            try:
                import yaml
                ...
                debug_log("SettingsModel", "_load_current_values", "Valores carregados: URL=%s, Email=%s", 
                         bool(self._jira_base_url), bool(self._jira_email))
            except Exception as e:
                debug_log("SettingsModel", "_load_current_values", "Erro ao ler .jira-config.yml: %s", e)
                pass  # Se não conseguir ler, usar valores vazios
        else:
            debug_log("SettingsModel", "_load_current_values", "Arquivo .jira-config.yml não encontrado")
    ...
    def is_configured(self) -> bool:
        ...
        return bool(
            self._jira_base_url and 
            self._jira_email and 
            self._jira_api_token and 
            self._account_id
        )
    ...
    def needs_configuration(self) -> bool:
        ...
        debug_log("SettingsModel", "needs_configuration", 
                 "config.json existe: %s, .jira-config.yml existe: %s", 
                 config_exists, jira_config_exists)
        ...
        debug_log("SettingsModel", "needs_configuration", 
                 ...
                 bool(self._jira_base_url), bool(self._jira_email), 
                 bool(self._jira_api_token), bool(self._account_id))
        ...
        debug_log("SettingsModel", "needs_configuration", "Precisa configurar: %s", needs_config)
        return needs_config
    ...
    class _AccountIdWorker(QThread):
        ...
        def run(self):
            ...
            try:
                ...
                debug_log("SettingsModel._AccountIdWorker", "run", "Buscando accountId em %s", url)
                debug_log("SettingsModel._AccountIdWorker", "run", "Email: %s", self._email)
                debug_log("SettingsModel._AccountIdWorker", "run", "Token presente: %s", bool(self._api_token))
                
                auth = HTTPBasicAuth(self._email, self._api_token)
                response = requests.get(url, auth=auth, timeout=10)
                
                debug_log("SettingsModel._AccountIdWorker", "run", "Response status=%d", response.status_code)

                if response.status_code == 200:
                    data = response.json()
                    account_id = data.get('accountId', '')
                    debug_log("SettingsModel._AccountIdWorker", "run", "accountId obtido: %s", account_id)
                    if account_id:
                        self.accountIdFetched.emit(account_id)
                    else:
                        debug_log("SettingsModel._AccountIdWorker", "run", "accountId não encontrado na resposta")
                        self.errorOccurred.emit("accountId não encontrado na resposta da API")
                else:
                    debug_log("SettingsModel._AccountIdWorker", "run", "Erro HTTP %d", response.status_code)
                    debug_log("SettingsModel._AccountIdWorker", "run", "Response: %s", response.text[:200])
                    self.errorOccurred.emit(f"Erro ao buscar accountId: HTTP {response.status_code}")
            except Exception as e:
                import traceback
                from src.utils.debug import debug_log
    ...
    def _start_fetch_account_id(self, server_url: str, email: str, api_token: str):
        ...
        debug_log("SettingsModel", "_start_fetch_account_id", "Iniciando busca de accountId")
        debug_log("SettingsModel", "_start_fetch_account_id", "URL=%s, Email=%s", server_url, email)
        
        # Cancelar worker anterior se existir
        if self._account_id_worker and self._account_id_worker.isRunning():
            debug_log("SettingsModel", "_start_fetch_account_id", "Cancelando worker anterior")
        ...
        self._account_id_worker = self._AccountIdWorker(server_url, email, api_token, self)
    ...
    def _on_account_id_fetched(self, account_id: str):
        ...
        debug_log("SettingsModel", "_on_account_id_fetched", "Recarregando valores após accountId ser obtido")
        self._load_current_values()
    ...
    def save(self):
        ...
        try:
            ...
            debug_log("SettingsModel", "save", "Criando arquivos de config se necessário")
            self._create_config_from_templates()

            # 2. Salvar no .jira-config.yml
            if not self._jira_base_url or not self._jira_email or not self._jira_api_token:
